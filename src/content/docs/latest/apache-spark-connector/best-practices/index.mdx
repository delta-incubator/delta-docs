...
<Aside type="note">
  ...
  [VACUUM](/latest/delta-utility#delta-vacuum) command.
</Aside>
...
If you don't need to change the table schema, you can [delete](/latest/delta-update#delete-from-a-table) data from a Delta table and insert your new data, or [update](/latest/delta-update#update-a-table) the table to fix the incorrect values.
...
- The old version of the table still exists. If you delete the wrong table you can easily retrieve the old data using [Time Travel](/latest/delta-batch#query-an-older-snapshot-of-a-table-time-travel).
...
In addition, if you want to delete old files to save storage cost after overwriting the table, you can use [VACUUM](/latest/delta-utility#delta-vacuum) to delete them. It's optimized for file deletion and usually faster than deleting the entire directory.
