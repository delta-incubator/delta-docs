...
For more examples, see [Rename columns](/latest/apache-spark-connector/table-batch-operations/#rename-columns).
...
For more details, see [Drop columns](/latest/apache-spark-connector/table-batch-operations/#drop-columns).
...
- Enabling column mapping on tables might break downstream operations that rely on Delta change data feed. See [Change data feed limitations for tables with column mapping enabled](/latest/apache-spark-connector/change-data-feed/#change-data-feed-limitations-for-tables-with-column-mapping-enabled).
...
- In Delta Lake 3.0 and above, [Spark Structured Streaming](https://spark.apache.org/docs/latest/structured-streaming-programming-guide.html) reads require schema tracking to be enabled on a column mapping enabled table that underwent column renaming or column dropping. See [Tracking non-additive schema changes](/latest/apache-spark-connector/table-streaming/#schema-tracking)
