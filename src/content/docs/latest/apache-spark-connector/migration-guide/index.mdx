...
- **External readers**: Directly reading the data stored in Delta Lake. For information on how to read Delta tables, see [Integrations](/latest/integrations).
...
For details, see [Convert a Parquet table to a Delta table](/latest/delta-utility#convert-a-parquet-table-to-a-delta-table).
...
If you are using `DeltaTable` APIs in Scala, Java, or Python to [update](/latest/delta-update) or [run utility operations](/latest/delta-utility) on them, then you may have to add the following configurations when creating the `SparkSession` used to perform those operations.
