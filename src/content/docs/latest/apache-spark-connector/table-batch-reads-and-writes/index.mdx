...
CREATE OR REPLACE TABLE default.people10m ( id INT, firstName STRING, middleName STRING, lastName STRING, gender STRING, birthDate TIMESTAMP, ssn STRING, salary INT ) USING DELTA`} lang="sql" />
...
DeltaTable.createOrReplace(spark) \\ .addColumn("id", "INT") \\ .addColumn("firstName", "STRING") \\ .addColumn("middleName", "STRING") \\ .addColumn("lastName", "STRING", comment = "surname") \\ .addColumn("gender", "STRING") \\ .addColumn("birthDate", "TIMESTAMP") \\ .addColumn("ssn", "STRING") \\ .addColumn("salary", "INT") \\ .property("description", "table with people data") \\ .location("/tmp/delta/people10m") \\ .execute()`} lang="python" />
...
// Create or replace table with path and add properties DeltaTable.createOrReplace(spark) .addColumn("id", "INT") .addColumn("firstName", "STRING") .addColumn("middleName", "STRING") .addColumn("lastName", "STRING", comment = "surname") .addColumn("gender", "STRING") .addColumn("birthDate", "TIMESTAMP") .addColumn("ssn", "STRING") .addColumn("salary", "INT") .property("description", "table with people data") .location("/tmp/delta/people10m") .execute()`} lang="scala" />
...
DeltaTable.create(spark) \\ .tableName("default.people10m") \\ .addColumn("id", "INT") \\ .addColumn("firstName", "STRING") \\ .addColumn("middleName", "STRING") \\ .addColumn("lastName", "STRING", comment = "surname") \\ .addColumn("gender", "STRING") \\ .addColumn("birthDate", "TIMESTAMP") \\ .addColumn("ssn", "STRING") \\ .addColumn("salary", "INT") \\ .partitionedBy("gender") \\ .execute()`} lang="python" />
...
// Using DeltaTable builder DeltaTable.createOrReplace(spark) .tableName("default.people10m") .addColumn("id", "INT") .addColumn("firstName", "STRING") .addColumn("middleName", "STRING") .addColumn("lastName", "STRING", comment = "surname") .addColumn("gender", "STRING") .addColumn("birthDate", "TIMESTAMP") .addColumn("ssn", "STRING") .addColumn("salary", "INT") .partitionedBy("gender") .execute()`} lang="scala" />
...
DeltaTable.create(spark) .tableName("table_name") .addColumn( DeltaTable.columnBuilder(spark, "id_col1") .dataType(LongType) .generatedAlwaysAsIdentity().build()) .addColumn( DeltaTable.columnBuilder(spark, "id_col2") .dataType(LongType) .generatedAlwaysAsIdentity(start = -1L, step = 1L).build()) .addColumn( DeltaTable.columnBuilder(spark, "id_col3") .dataType(LongType) .generatedByDefaultAsIdentity().build()) .addColumn( DeltaTable.columnBuilder(spark, "id_col4") .dataType(LongType) .generatedByDefaultAsIdentity(start = -1L, step = 1L).build()) .execute()`} lang="scala" />
...
old_table_df = spark.table("old_table").select("id", "event_date", "some_value") old_table_df.write .format("delta") .mode("append") .saveAsTable("new_table")

# Insert records and generate new IDs

new_records_df = spark.table("new_records").select("event_date", "some_value") new_records_df.write .format("delta") .mode("append") .saveAsTable("new_table")`} lang="python" />
...
// Insert records including existing IDs val oldTableDF = spark.table("old_table").select("id", "event_date", "some_value") oldTableDF.write .format("delta") .mode("append") .saveAsTable("new_table")

// Insert records and generate new IDs val newRecordsDF = spark.table("new_records").select("event_date", "some_value") newRecordsDF.write .format("delta") .mode("append") .saveAsTable("new_table")`} lang="scala" />
...
-- If table is registered in the metastore SELECT \* FROM default.people10m;`} lang="sql" />
...
// Read table from metastore val df = spark.table("default.people10m")`} lang="scala" />
...
// Read table from metastore Dataset<Row> df = spark.table("default.people10m");`} lang="java" />
...
dataFrame.write.format(...).option("txnVersion", version).option("txnAppId", app_id).save(...)`} lang="python" />
...
dataFrame.write.format(...).option("txnVersion", version).option("txnAppId", appId).save(...)`} lang="scala" />
...
<Tabs>
  <TabItem label="SQL">
    <Code
      code={`ALTER TABLE table_name DROP COLUMN col_name`}
      lang="sql"
    />
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Python">
    <Code
      code={`df.write.option("overwriteSchema", "true")`}
      lang="python"
    />
  </TabItem>
</Tabs>
...
-- Show just the 'department' table property. SHOW TBLPROPERTIES default.people10m ('department');`} lang="sql" />
...
spark = SparkSession \\ .builder \\ .appName("...") \\ .master("...") \\ .config("spark.sql.extensions", "io.delta.sql.DeltaSparkSessionExtension") \\ .config("spark.sql.catalog.spark_catalog", "org.apache.spark.sql.delta.catalog.DeltaCatalog") \\ .getOrCreate()`} lang="python" />
...
val spark = SparkSession .builder() .appName("...") .master("...") .config("spark.sql.extensions", "io.delta.sql.DeltaSparkSessionExtension") .config("spark.sql.catalog.spark_catalog", "org.apache.spark.sql.delta.catalog.DeltaCatalog") .getOrCreate()`} lang="scala" />
...
SparkSession spark = SparkSession .builder() .appName("...") .master("...") .config("spark.sql.extensions", "io.delta.sql.DeltaSparkSessionExtension") .config("spark.sql.catalog.spark_catalog", "org.apache.spark.sql.delta.catalog.DeltaCatalog") .getOrCreate();`} lang="java" />
...
For detailed information about Hadoop file system configurations for your specific storage, refer to the [Storage configuration](delta-storage.html) documentation.
