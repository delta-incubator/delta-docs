...
spark.readStream.delta("/tmp/delta/events")`} lang="python" />
...
<Aside type="note">
  ...
  `logRetentionDuration` [configuration](delta-batch.html#-data-retention) and
...
batch_df.write.format(...).option("txnVersion", batch_id).option("txnAppId", app_id).save(...) # location 2`} lang="python" />
