...
- The table must have column mapping enabled. See [Delta column mapping](/latest/delta-column-mapping).
...
Enabling Delta UniForm Iceberg requires "delta-iceberg" to be provided to Spark shell: --packages io.delta:delta-iceberg_2.12:&lt;version&gt;

Enabling Delta UniForm Hudi requires "delta-hudi" to be provided to Spark shell: --packages io.delta:delta-hudi_2.12:&lt;version&gt;
...
- UniForm does not work on tables with deletion vectors enabled. See [What are deletion vectors?](/latest/delta-deletion-vectors).
